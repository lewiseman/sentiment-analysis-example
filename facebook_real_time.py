...
def sentimentAnalysis(sentencesComments):
    ...
    sumPos = 0
    sumNeg = 0
    for cnt, sentence in enumerate(sentencesComments, start=1):
...
def satisfyConditions(sentencesCommentsCurr, sentencesComments):
    return len(sentencesCommentsCurr) > len(sentencesComments)
...
def main_real_time_analysis():
    ...
    idpost = sys.argv[2]
    idpage = sys.argv[3]
    post_ids.append(f"{idpage}_{idpost}")
    graph = facebook.GraphAPI(token)
    posts = {
        'created_time' : '1625309472.357246'
    }
